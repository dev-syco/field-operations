import Operation, {
  ...
  OperationListSort,
  OperationType,
} from '@/services/fieldService/models/Operation';
...
export default class FieldService {
   ...
   */
  getOperations({ filterCallback, sort }: { filterCallback?: ((o: Operation) => boolean) | undefined, sort: OperationListSort }): Promise<Array<Operation>> {
    return new Promise((resolve) => {
      setTimeout(() => {
        const clonedOperations = this.operations.map((operation) => operation);
        const filtered = filterCallback ? clonedOperations.filter(filterCallback) : clonedOperations;
        resolve(orderBy(filtered, [sort?.field], [sort?.order]));
      }, 500);
    });
  }
  ...
  private getOperationSync(operationId: string | null = null): Operation | null {
    ...
    const operation = this.operations.find((i) => i.id === operationId);
  ...
  saveOperation(operation: Operation): Promise<Operation> {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (isExistingOperation) this.operations.splice(operationIndex, 1, operationToSave);
        else {
